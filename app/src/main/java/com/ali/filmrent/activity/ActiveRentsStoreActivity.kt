...
class ActiveRentsStoreActivity : AppCompatActivity() , RentEvents{
    ...
    private fun showData() {
        binding.toolbarFilm.title = "Active rents"
        val listActiveRentsOfStore: List<Rental> =
            rentalDao.listActiveRentsOfStore(store.store_id!!)

        binding.txtNumberActiveRents.text =
            "The number of Active rents : " + listActiveRentsOfStore.size

        val adapter = ActiveRentAdapter(
            rents = ArrayList(listActiveRentsOfStore),
